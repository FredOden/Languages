...
void demo3d() {
	...
	for (int i=0; i<360; i++) {
		...
		s = renderer.rotate(s, Lourah::geometry::toRadian(f*.06), Lourah::geometry::toRadian(f*.01), Lourah::geometry::toRadian(f*.02));
		s = renderer.translate(s, Lourah::geometry::AVector(0, 0, f/720));
		//std::cerr << i << " renderer.rotate" << std::endl;
		renderer.render(renderer.translate(
					renderer.rotate(t, Lourah::geometry::toRadian(0), Lourah::geometry::toRadian(f), Lourah::geometry::toRadian(f)),Lourah::geometry::AVector(0, 0, f)
					)
			       );
